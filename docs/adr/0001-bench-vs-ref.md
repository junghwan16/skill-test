...
`gisul bench` answers "does this skill help vs. no skill at all" — the
...
```
$ gisul bench code-review --vs main
...
- **Not a general git-diff tool.** No prose-diff rendering; `gisul` reports
...
gap — a future `gisul capture <skill> "<prompt>"` — left as its own piece
