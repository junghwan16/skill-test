...
`gisul` v1 nails **#1** — cheap, fast, the highest-frequency need. v2
ships **#2** as `gisul bench`: the same cases, run with and without the
...
- It **adopts the community `evals/cases.yaml` schema** (from the `skill-eval`
  skill) rather than inventing one. Cases stay portable; gisul is just the
...
```
gisul                    # discover & run every eval (**/*.eval.yaml, evals/cases.yaml)
gisul sql                # filter to a skill or a file
gisul -t "negative"      # filter by case id/substring
gisul --ci               # non-zero exit on any regression / unwritten case
gisul --json out.json    # machine-readable results alongside the grid
gisul bench sql          # A/B with vs without the skill; report the lift
gisul new sql            # scaffold what's missing: sql/SKILL.md and/or sql.eval.yaml
gisul lint [target...]   # validate SKILL.md files (errors) + guidance heuristics (warnings)
gisul fmt [--check]      # conservative SKILL.md frontmatter/whitespace normalizer
```
...
There is a single scaffolding command. `gisul new <skill>` creates
...
official `skill-creator` runs, packaged as one command: `gisul bench`.
...
```
gisul bench [target]           # with vs without, report lift
...
6. **Offline `validate`.** `gisul validate [target]` parses suites with no
...
8. **Every subcommand silently dropped its options.** The root command was a
   ...
   subcommand, so `gisul [target]` and bare `gisul` still work while
...
- **Overlapping installed siblings out-compete the example.** Pass 2 ran on a
  ...
  That is not a CLI defect — it's exactly the over-triggering collision gisul
...
The source is TypeScript (`tsc` → `dist/`, published as the `gisul` bin),
